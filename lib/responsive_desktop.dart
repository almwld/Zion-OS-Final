            ...
            children: [
                  ...
                  child: Column(
                    ...
                    children: [
                      Container(padding: const EdgeInsets.all(20), child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                        _buildCategoryButton('categories.attack', Colors.red, prefs, Icons.bug_report),
                        const SizedBox(width: 20),
                        _buildCategoryButton('categories.defense', Colors.blue, prefs, Icons.shield),
                        const SizedBox(width: 20),
                        _buildCategoryButton('categories.analysis', Colors.green, prefs, Icons.analytics),
                        const SizedBox(width: 20),
                        _buildCategoryButton('categories.tools', Colors.orange, prefs, Icons.build),
                      ])),
                      const SizedBox(height: 40),
                      Text('desktop.welcome', style: TextStyle(fontSize: 24 * prefs.fontScale, fontWeight: FontWeight.bold, color: prefs.isDarkMode ? Colors.white : Colors.black)),
                      const SizedBox(height: 20),
                      Text('desktop.description', style: TextStyle(fontSize: 14 * prefs.fontScale, color: prefs.isDarkMode ? Colors.white70 : Colors.black54)),
                    ],
                  ),
                ),
              ),
              Container(
                ...
                child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                  _buildDockIcon(Icons.terminal, 'apps.terminal', prefs),
                  const SizedBox(width: 20),
                  _buildDockIcon(Icons.wifi, 'apps.wifi_scanner', prefs),
                  const SizedBox(width: 20),
                  _buildDockIcon(Icons.security, 'apps.security_hub', prefs),
                  const SizedBox(width: 20),
                  _buildDockIcon(Icons.folder, 'apps.file_manager', prefs),
                  const SizedBox(width: 20),
                  _buildDockIcon(Icons.settings, 'apps.settings', prefs, onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const SettingsScreen()))),
                ]),
              ),
            ],
          ),
